    ...
    def __init__(self):
        ...
        with self.graph.as_default():
            ...
            with tf.variable_scope('output'):
                weights = tf.get_variable('weights', [input_size, output_size], tf.float32, tf.zeros_initializer())
                biases = tf.get_variable('biases', [output_size], tf.float32, tf.zeros_initializer())
                self.params.extend([weights, biases])
                self.probs = tf.nn.softmax(tf.matmul(self.images, weights) + biases)

            self.error = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.targets, logits=tf.matmul(self.images, weights) + biases))
