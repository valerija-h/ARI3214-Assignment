...
class Model(object):
    ...
    def __init__(self):
        ...
        with self.graph.as_default():
            ...
            #Define model here
            ################################################
            # How narrow you want the bell curve - to pick smaller numbers (close to 0 so weights are smallers)
            with tf.variable_scope('hidden'):
                weights = tf.get_variable('weights', [input_size, hidden_layer_size], tf.float32, tf.random_normal_initializer(stddev=init_stddev))
                biases = tf.get_variable('biases', [hidden_layer_size], tf.float32, tf.zeros_initializer())
                self.params.extend([ weights, biases ])
                self.hprobs = tf.nn.dropout(tf.sigmoid(tf.matmul(self.images, weights) + biases), dropout_keep_prob)
            
            with tf.variable_scope('output'):
                weights = tf.get_variable('weights', [hidden_layer_size, output_size], tf.float32, tf.random_normal_initializer(stddev=init_stddev))
                biases = tf.get_variable('biases', [output_size], tf.float32, tf.zeros_initializer())
                self.params.extend([weights, biases])
                self.probs = tf.nn.softmax(tf.matmul(self.hprobs, weights) + biases)

            self.error = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.targets, logits=tf.matmul(self.hprobs, weights) + biases))
